...
function DNS42_Migrations_1ChangeManaged_up($params=null) {
	...
	$sql['MySQL'] = 'ALTER TABLE '.$table." ADD `maestra` TINYINT(1) NOT NULL DEFAULT '1' AFTER `delegacion`, ADD `reversa` TINYINT(1) NOT NULL DEFAULT '0' AFTER `maestra`";
	$db = Gatuf::db();
...
function DNS42_Migrations_1ChangeManaged_down($params=null) {
	$table = Gatuf::factory('DNS42_ManagedDomain')->getSqlTable();
	$sql = array();
	//$sql['PostgreSQL'] = 'ALTER TABLE '.$table.' DROP COLUMN "maestra", DROP COLUMN "reversa"';
	$sql['MySQL'] = 'ALTER TABLE '.$table.' DROP COLUMN `maestra`, DROP COLUMN `reversa`';
	$db = Gatuf::db();
	$engine = Gatuf::config('db_engine');
	if (!isset($sql[$engine])) {
		throw new Exception($engine.' migration not supported.');
