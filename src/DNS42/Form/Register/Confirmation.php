 ...
 *
 */
class DNS42_Form_Register_Confirmation extends Gatuf_Form {
	public $_user = null;
	...
	public function clean_key() {
		...
		if (false === ($email_id = DNS42_Form_Register_InputKey::checkKeyHash($this->cleaned_data['key']))) {
		...
		if ($users[0]->active) {
			throw new Gatuf_Form_Invalid(__('This account has already been confirmed. Maybe should you try to recover your password using the help link.'));
		}
	...
	function save($commit=true) {
		...
		if ($commit) {
			...
			Gatuf_Signal::send('Gatuf_User::passwordUpdated',
							  'DNS42_Form_Register_Confirmation', $params);
		}
		return $this->_user;
	}
}
