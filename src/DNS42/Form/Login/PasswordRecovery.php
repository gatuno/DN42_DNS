...
class DNS42_Form_Login_PasswordRecovery extends Gatuf_Form {
	...
	function save ($commit=true) {
		...
		foreach ($users as $user) {
			if ($user->active) {
				$return_url = Gatuf_HTTP_URL_urlForView ('DNS42_Views_Login::passwordRecoveryInputCode');
				$tmpl = new Gatuf_Template('dns42/login/recuperarcontra-email.txt');
				...
				$url = Gatuf::config ('url_base').Gatuf_HTTP_URL_urlForView ('DNS42_Views_Login::passwordRecovery', array ($code), array (), false);
				$urlic = Gatuf::config ('url_base').Gatuf_HTTP_URL_urlForView ('DNS42_Views_Login::passwordRecoveryInputCode', array (), array (), false);
