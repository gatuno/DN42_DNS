...
class DNS42_Views_Users {
	...
	public function index ($request, $match) {
		...
		$pag->action = array ('DNS42_Views_Users::index');
		...
		$pag->edit_action = array ('DNS42_Views_Users::ver', 'id');
	...
	public function agregar ($request, $match) {
		...
		if ($request->method == 'POST') {
			$form = new DNS42_Form_User_Agregar ($request->POST);
			
			if ($form->isValid ()) {
				$n_user = $form->save ();
				
				$url = Gatuf_HTTP_URL_urlForView ('DNS42_Views_Users::ver', array ($n_user->id));
	...
	public function cambiar_pass ($request, $match) {
		...
		if ($request->method == 'POST') {
			$form = new DNS42_Form_User_ChangeOwnPassword ($request->POST, array ('user' => $request->user));
			
			if ($form->isValid ()) {
				$form->save ();
				
				$url = Gatuf_HTTP_URL_urlForView ('DNS42_Views_Users::ver', array ($request->user->id));
				return new Gatuf_HTTP_Response_Redirect ($url);
			}
		} else {
	...
	public function reset_pass ($request, $match) {
		...
		if ($user->id == $request->user->id) {
			$url = Gatuf_HTTP_URL_urlForView ('DNS42_Views_Users::cambiar_pass');
			
			return new Gatuf_HTTP_Response_Redirect ($url);
		}
		
		if ($request->method == 'POST') {
			$form = new DNS42_Form_User_ChangePassword ($request->POST, array ('user' => $user));
			
			if ($form->isValid ()) {
				$form->save ();
				
				$url = Gatuf_HTTP_URL_urlForView ('DNS42_Views_Users::ver', array ($user->id));
				return new Gatuf_HTTP_Response_Redirect ($url);
			}
		} else {
	...
	public function actualizar ($request, $match) {
		...
		if ($request->method == 'POST') {
			$form = new DNS42_Form_User_Actualizar ($request->POST, array ('user' => $user));
