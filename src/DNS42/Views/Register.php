...
class DNS42_Views_Register {
	function register ($request, $match) {
		...
		if ($request->method == 'POST') {
			...
			if ($form->isValid ()) {
				...
				$url = Gatuf_HTTP_URL_urlForView('DNS42_Views_Register::inputKey');
	...
	function inputKey ($request, $match) {
		...
		if ($request->method == 'POST') {
			$form = new DNS42_Form_Register_InputKey($request->POST);
			if ($form->isValid()) {
				$key = $form->save();
				$url = Gatuf_HTTP_URL_urlForView('DNS42_Views_Register::confirmation', array ($key));
				return new Gatuf_HTTP_Response_Redirect($url);
			}
		} else {
			$form = new DNS42_Form_Register_InputKey(null);
		}
		return Gatuf_Shortcuts_RenderToResponse('dns42/register/inputkey.html', 
		                                         array('page_title' => $title,
		                                         'form' => $form),
		                                         $request);
	}
	...
	function confirmation ($request, $match) {
		...
		$email_id = DNS42_Form_Register_InputKey::checkKeyHash($key);
		if (false == $email_id) {
			$url = Gatuf_HTTP_URL_urlForView('DNS42_Views_Register::inputKey');
			return new Gatuf_HTTP_Response_Redirect($url);
		}
		...
		if ($request->method == 'POST') {
			$form = new DNS42_Form_Register_Confirmation($request->POST, $extra);
			if ($form->isValid()) {
				$user = $form->save();
				$request->user = $user;
			...
			$form = new DNS42_Form_RegisterConfirmation(null, $extra);
		}
		return Gatuf_Shortcuts_RenderToResponse('dns42/register/confirmation.html', 
		                                         array('page_title' => $title,
		                                         'new_user' => $user,
		                                         'form' => $form),
		                                         $request);
