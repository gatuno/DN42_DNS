 ...
 */
function DNS42_Shortcuts_RenderToResponse($tplfile, $params, $request=null) {
	if (!is_null ($request) && $request->theme !== false) {
		/* Cuando el tema no es el default, agregar la carpeta del tema */
		$skin = dirname(__FILE__).'/themes/'.$request->theme;
		$folders = array_merge (array ($skin), Gatuf::config ('template_folders', array ()));
		$tmpl = new Gatuf_Template($tplfile, $folders);
	} else {
